...
for (my $i = 0; $i < $bt_test_sub_count; $i++) {
    $bt_test_subs[$i] = sub {
        ...
        if (++$depth < $bt_test_sub_count) {
            &{$bt_test_subs[$depth]}($depth);   # *01* &caller_fixed(1..count-1) point here.
        } else {
            foreach my $frame (0..$depth) {
                my @my_caller = &caller_fixed($frame);  # *02* &caller_fixed(0) points here.
                ...
                if ($frame == $depth) {
                    ...
                    is($my_caller[2], __LINE__ + 20, "&caller_fixed() returns correct line number (frame $frame)");
                    ...
                    is($my_caller[2], __LINE__ - (($frame) ? (12) : (9)), "&caller_fixed() returns correct line number (frame $frame)");
                    is($my_caller[3], "__ANON__()", "&caller_fixed() returns correct subroutine name (frame $frame)");
                }
            }
            # Test &get_backtrace() while we're at it.
            my @bt = &get_backtrace();  # *03* The backtrace will point here.
            ...
            foreach my $frame (0..$bt_test_sub_count) {
                ...
                like($bt[$frame], qr/^[ ]{$frame}\[$frame\] $file:(87|104|115) \| (MAIN|__ANON__)\(\)$/,
                     "Backtrace has correct format");
            }
        }
    };
}
&{$bt_test_subs[0]}(0);  # *04* &caller_fixed($bt_test_sub_count) points here.
